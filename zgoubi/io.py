...
def define_file(fname, allow_lookup=False):
	...
	for rname, rtype in zip(col_names, col_types):
		...
		if rtype == "int":
			ntype = "i4"
			nbytes = 4
		if rtype == "string" or rtype =='sting':
			if rname == "KLEY":
				ntype = "U10"
				nbytes = 10
			if rname == "LABEL1":
			...
			if rname == "LET":
				...
				nbytes = 1
			if rname == 'DATE':
				#CDK, deal with other data and time somehow
				ntype="U10"
				nbypes=10				
			if rname == 'TIME':
				#CDK, deal with other data and time somehow
		...
		try:
			...
			for n, row in enumerate(file_data):
				try:
					...
					#CDK 02-02-2021 crude fix to deal with date and time appearing in the new SVN version of Zgoubi file
					#CDK 31-03-2021, fix is modified to avoid changing last two columns of zgoubi.plt file. Only applies to zgoubi.fai			
					#if "-" in new_row[-2]:
					#	new_row[-2] = 0
					#	new_row[-1] = 0
					#CDK 02-02-2021 crude fix to deal with len of new_row being less than len(data_type) in fai file
					if len(new_row) == len(data_type) - 1:
						new_row.append(0)

					#print ("new_row ",new_row, "len ",len(new_row), "len(data_type) ",len(data_type))
